...
const GameManual: React.FC = () => {
  ...
  const getCharacters = (): Character[] => [
    {
      name: 'Samson',
      role: t.characters.samson.role,
      description: t.characters.samson.description,
      color: 'green',
    ...
    {
      name: 'Michael',
      role: t.characters.michael.role,
      description: t.characters.michael.description,
      color: 'green',
    ...
    {
      name: 'Max and Stacey',
      role: t.characters.maxStacey.role,
      description: t.characters.maxStacey.description,
      color: 'green',
    ...
    {
      name: 'Nayib',
      role: t.characters.nayib.role,
      description: t.characters.nayib.description,
      color: 'green',
    ...
    {
      name: 'Adam',
      role: t.characters.adam.role,
      description: t.characters.adam.description,
      color: 'green',
    ...
    {
      name: 'Faketoshi',
      role: t.characters.faketoshi.role,
      description: t.characters.faketoshi.description,
      color: 'green',
    ...
    {
      name: 'Saifedean',
      role: t.characters.saifedean.role,
      description: t.characters.saifedean.description,
      color: 'green',
  ...
  return (
    <div className="space-y-16">
      ...
      */}
      <div className="bg-yellow-400 border-4 border-black p-6 md:p-8 relative overflow-hidden pixel-shadow">
        ...
        */}
        ...
        >
          ...
          <span className="text-sm">{t.hero.buyAtCopiaro}</span>
        </a>
        <p className="text-center text-yellow-800 text-[10px] mt-2 font-mono">
          {t.hero.copiaroSubtext}
        </p>
      </div>
      ...
      {/* SECTION 3: CONTROLS */}
      <div>
         <h2 className="font-pixel text-xl border-b-4 border-black mb-6 pb-2">{t.controls.title}</h2>
         <div className="flex flex-col md:flex-row gap-8 items-center justify-center">
             ...
             */}
             ...
             <div className="grid grid-cols-1 gap-4 w-full max-w-sm">
                {[
                    { btn: t.controls.aButton, desc: t.controls.aDesc, color: 'bg-red-500' },
                    { btn: t.controls.bButton, desc: t.controls.bDesc, color: 'bg-red-500' },
                    { btn: t.controls.start, desc: t.controls.startDesc, color: 'bg-black' },
                    { btn: t.controls.select, desc: t.controls.selectDesc, color: 'bg-black' },
                    { btn: t.controls.dPad, desc: t.controls.dPadDesc, color: 'bg-black' }
                ].map((c) => (
      ...
      <div>
         <h2 className="font-pixel text-xl border-b-4 border-black mb-6 pb-2">{t.characters.title}</h2>
      ...
      <div className="grid md:grid-cols-2 gap-12 pt-8 border-t-4 border-black">
         <div>
            <h3 className="font-pixel text-lg mb-4">{t.tips.title}</h3>
            <ul className="space-y-6">
                <li className="flex gap-4">
                    <div className="w-8 h-8 bg-black text-white flex items-center justify-center font-bold font-serif shrink-0">?</div>
                    <div className="text-sm">
                        <strong className="block font-bold mb-1">{t.tips.collectingBitcoin}</strong>
                        {t.tips.collectingDesc}
                    </div>
                </li>
                <li className="flex gap-4">
                    <div className="w-8 h-8 bg-black text-white flex items-center justify-center font-bold font-serif shrink-0">!</div>
                    <div className="text-sm">
                        <strong className="block font-bold mb-1">{t.tips.gettingStuck}</strong>
                        {t.tips.stuckDesc}
                    </div>
                </li>
            </ul>
         ...
         <div>
            <h3 className="font-pixel text-lg mb-4">{t.credits.title}</h3>
            <div className="bg-gray-100 p-6 font-mono text-xs border-2 border-black pixel-shadow-sm">
                <dl className="grid grid-cols-2 gap-y-3">
                    <dt className="text-gray-500">{t.credits.pixelart}</dt>
                    <dd className="font-bold">Hero of Bitcoin</dd>

                    <dt className="text-gray-500">{t.credits.programming}</dt>
                    <dd className="font-bold">Hero of Bitcoin</dd>

                    <dt className="text-gray-500">{t.credits.music}</dt>
                    <dd className="font-bold">Mr. V</dd>

                    <dt className="text-gray-500">{t.credits.soundFx}</dt>
      ...
      <div className="text-[10px] text-gray-400 font-mono leading-relaxed border-t border-gray-200 pt-6">
        ...
        <p>
          {t.legal.disclaimer2} <strong className="text-gray-500">{t.legal.notLicensed}</strong>
        </p>
      </div>
